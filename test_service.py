...
endpoint = "https://data.rtt.io/gb-nr/service/gb-nr/G26236/2026-08-04"
req = urllib.request.Request(endpoint, headers={'Authorization': f'Bearer {access_token}'}, method='GET')
try:
    res = urllib.request.urlopen(req, context=ctx)
    data = json.loads(res.read().decode())
    locations = data.get('locations', [])
    print(f"Found {len(locations)} calling points")
    if len(locations) > 0:
        print("First 3 stops:")
        for loc in locations[:3]:
            print(loc['location']['shortCodes'][0], loc.get('temporalData', {}))
